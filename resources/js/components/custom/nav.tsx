...
const Nav = () => {
    ...
    const notifications = [
    { id: 1, post_id: 1, text: "Novi komentar od Marko Jovanovic na vašem postu Markaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa", time: "5 min ago", is_read: true },
    { id: 2, post_id: 2, text: "Diskusija \"Da li je velicina bitna?\" koju ste pratili je zatvorena.", time: "10 min ago", is_read: true },
    { id: 3, post_id: 3, text: "Novi odgovor od Ana Nikolic na vašem postu Post broj tri", time: "30 min ago", is_read: true },
    ...
    <nav className="z-50 sticky top-0 left-0 flex justify-between items-center w-full h-[60px] px-4 font-jersey bg-primary *:text-secondary a:font-jersey border-b border-secondary/10 ease-in-out transition-all duration-300">
        ...
        <ul className={`w-10/12 min-h-[calc(100vh-60px)] md:min-h-auto bg-primary md:bg-transparent md:w-auto md:h-auto fixed md:static top-[60px] md:top-0 left-full md:left-0 flex flex-col md:flex-row items-center md:gap-8 *:text-secondary transition-all duration-300 shadow-2xl ${isMenuOpen ? "openMenu" : ""}`}>
            {!user ? (
            <>
                <li><Link href="/login" className="font-jersey block w-full h-fit md:w-auto p-4 md:p-0 bg-primary md:bg-transparent border-b md:border-0 border-secondary/10 hover:opacity-50">Login</Link></li>
                <li><Link href="/register" className="font-jersey block w-full h-fit md:w-auto p-4 md:p-0 bg-primary md:bg-transparent border-b md:border-0 border-secondary/10 hover:opacity-50">Register</Link></li>
            </>
            ) : (
            <>
                {/* dashboard */}
                ...
                <li className="w-full">
                            ...
                            >
                                {notifications.map((notification: NotificationProps) => (
                                    ...
                                    <DropdownMenuLabel className="mt-auto absolute bottom-0 left-0 z-10 w-full h-10 p-0 flex justify-between items-center rounded-b-2xl overflow-hidden backdrop-blur-3xl">
                                        <Link href="/notifications" className="bg-accent-blue w-full h-full px-4 py-2 flex justify-center items-center text-center text-secondary rounded-none cursor-pointer hover:opacity-100 hover:bg-accent-blue/75">
                                            See all notifications
                                        </Link>

                                        <Button className="bg-accent-lime w-full h-full px-4 py-2 flex justify-center items-center text-center text-secondary rounded-none cursor-pointer hover:bg-accent-lime/75" onClick={() => console.log("Mark all as read")}>
                                            Mark all as read
                                        </Button>
                                    </DropdownMenuLabel>
                            </DropdownMenuContent>
                    ...
                    <Link href="/notifications" className="block md:hidden font-jersey w-full h-fit md:w-auto p-4 md:p-0 bg-primary md:bg-transparent border-b md:border-0 border-secondary/10">
                        Notifications
                    </Link>
                </li>
